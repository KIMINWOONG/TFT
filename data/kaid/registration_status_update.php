...
if($registration['cr_status']!=$status && $status=="cancelled" && $registration['cr_payment_method']=="카드" && $registration['cr_receipt_price']>0){
...
$sql="update g5_conference_registration set cr_status='{$status}', cr_update_date=now() where cr_id='{$cr_id}'";
sql_query($sql);
