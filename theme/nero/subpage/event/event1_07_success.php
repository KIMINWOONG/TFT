...
if (false && $calculatedAmount != $amount) {
	...
	alert("결제 금액이 일치하지 않습니다.",G5_THEME_URL.'/subpage/event/event1_07.php');
...
/**회비 납부 처리, 결제 데이터 저장***************/
    ...
    echo "<script>
        ...
        location.href = '" . G5_THEME_URL . "/subpage/event/event1_08.php';
    </script>";
...
/*****************/
    ...
    alert("결제 실패: " . $errorMessage, G5_THEME_URL.'/subpage/event/event1_07.php');
}
...
function processPaymentSuccess($orderData, $paymentResponse, $selectedItems) {
    ...
    try {
		...
		$sql = "INSERT INTO g5_conference_registration SET
				...
				cr_payment_date = NOW(),
				cr_receipt_price = '".$paymentResponse['totalAmount']."',
        ...
        // 에러 처리
        alert("결제 처리 중 오류가 발생했습니다: " . $e->getMessage(), G5_THEME_URL.'/subpage/event/event1_07.php');
        exit;
    }
}
