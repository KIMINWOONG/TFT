   ...
   <div class="sub_menu">
	...
	<table class="meminfo">
        ...
        <tr>
            ...
            <td>
                <?php 
                if ($member['mb_bachelor_admission_year'] || $member['mb_bachelor_graduation_year']) {
                    echo ($member['mb_bachelor_admission_year'] ? $member['mb_bachelor_admission_year'] : '-');
                    echo ' ~ ';
                    echo ($member['mb_bachelor_graduation_year'] ? $member['mb_bachelor_graduation_year'] : '-');
                } else {
                    echo '-';
                }
                ?>
        ...
        <tr>
            ...
            <td>
               <?php 
                if ($member['mb_graduate_data']) {
                    $graduate_data = json_decode($member['mb_graduate_data'], true);
                    if ($graduate_data && is_array($graduate_data)) {
                        $graduate_list = array();
                        foreach ($graduate_data as $data) {
                            if ($data['degree'] || $data['school_major']) {
                                $graduate_info = '';
                                if ($data['degree']) {
                                    $graduate_info .= $data['degree'];
                                }
                                if ($data['school_major']) {
                                    $graduate_info .= ($graduate_info ? ' - ' : '') . $data['school_major'];
                                }
                                if ($data['admission_year'] || $data['graduation_year']) {
                                    $graduate_info .= ' (';
                                    $graduate_info .= ($data['admission_year'] ? $data['admission_year'] : '-');
                                    $graduate_info .= ' ~ ';
                                    $graduate_info .= ($data['graduation_year'] ? $data['graduation_year'] : '-');
                                    $graduate_info .= ')';
                                }
                                $graduate_list[] = $graduate_info;
                            }
                        }
                        echo $graduate_list ? implode('<br>', $graduate_list) : '-';
                    } else {
                        echo '-';
                    }
                } else {
        ...
        <tr>
            ...
            <td>
                <?php 
                if ($member['mb_mail_address_type']) {
                    echo $member['mb_mail_address_type'] == '자택' ? '자택 (Home)' : '근무지 (Workplace)';
                } else {
                    echo '-';
                }
                ?>
        ...
        <tr>
            <th>재직여부</th>
            <td><?=$member['mb_employment_status']?></td>
        </tr>
        <tr>
            <th>근무지</th>
