...
// 전체 개수 (페이징용)
$total_sql = "SELECT COUNT(*) as cnt FROM g5_council $where_sql";
...
<div class="officers common">
    <div class="width">
        <h2 class="contents_title">대한치과이식임플란트학회 평의원 및 감사 소개</h2>
                ...
                <div class="tab_content" id="content-list">
                    <div class="items">
                        ...
                        if (sql_num_rows($result) == 0) {
                            echo '<div class="no_data">등록된 평의원이 없습니다.</div>';
                        } else {
                            while ($row = sql_fetch_array($result)) {
                        ?>
                        <a href="javascript:void(0)" class="profile_popup item" data-id="<?php echo $row['co_id']; ?>">
                            <?php if ($row['co_image']) { ?>
                                <img src="<?php echo G5_DATA_URL; ?>/council/<?php echo $row['co_image']; ?>" alt="<?php echo $row['co_name']; ?>">
                            <?php } else { ?>
                                <img src="<?php echo G5_THEME_IMG_URL ?>/profile.png" alt="<?php echo $row['co_name']; ?>">
                            <?php } ?>
                            <div class="name_wrap">
                                <h5><?php echo $row['co_department']; ?></h5>
...
$(function () {
    ...
    $(document).on("click", ".profile_popup", function () {
        const council_id = $(this).data("id");
        loadCouncilDetail(council_id);
    });
...
// 평의원 상세 정보 로드
function loadCouncilDetail(council_id) {
    $.ajax({
        url: './council_ajax.php',
        type: 'GET',
        data: { 
            mode: 'detail',
            co_id: council_id 
        ...
        success: function(data) {
            if (data.success) {
                ...
                // 이미지 설정
                if (council.co_image) {
                    $('#popup_image').attr('src', '<?php echo G5_DATA_URL; ?>/council/' + council.co_image);
                } else {
                    $('#popup_image').attr('src', '<?php echo G5_THEME_IMG_URL ?>/profile.png');
                }
                
                // 기본 정보 설정
                $('#popup_category').text(council.co_category + ', 평의원');
                $('#popup_name').text(council.co_name );
                
                // 경력 정보 설정
                $('#popup_career').empty();
                if (council.co_career) {
                    const careers = council.co_career.split('\n');
