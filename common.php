...
ini_set('display_errors', '1'); // 에러를 브라우저에 출력
...
// 회원, 비회원 구분
$is_member = $is_nonemember = $is_guest = false;
$is_admin = '';
if (isset($member['mb_id']) && $member['mb_id']) {
	...
	if (isset($_SESSION['ss_nonemb_name']) && $_SESSION['ss_nonemb_name']) {
		$is_nonemember = true;
	}else{
		$is_guest = true;
		$member['mb_id'] = '';
...
if(isset($_GET['theme']) && $_GET['theme']){
    set_session("theme", $_GET['theme']);
}

if(get_session("theme")){
    $new_theme=get_session("theme");
    $config['cf_theme']=$new_theme;
