...
function get_paging_new($write_pages, $cur_page, $total_page, $url, $add="")
{
	...
	$str = '';
	$str .= '<a href="'.$url.(1).$add.'" class="prev_2"><img src="'.G5_THEME_IMG_URL.'/prev2.png" alt=""></a>';
    ...
    if ($start_page > 1) $str .= '<a href="'.$url.($start_page-1).$add.'" class="prev_1"><img src="'.G5_THEME_IMG_URL.'/prev1.png" alt=""></a>'.PHP_EOL;

    $str .= '<ul>';
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= '<li><a href="'.$url.$k.$add.'" class="page_active">'.$k.'<span class="sound_only">페이지</span></a></li>'.PHP_EOL;
            else
                $str .= '<li><a href="#">'.$k.'</a></li>'.PHP_EOL;
        }
    $str .= '</ul>';

    if ($total_page > $end_page) $str .= '<a href="'.$url.($end_page+1).$add.'" class="next_1"><img src="'.G5_THEME_IMG_URL.'/next1.png" alt=""></a>'.PHP_EOL;

	$str .= '<a href="'.$url.$total_page.$add.'" class="next_2"><img src="'.G5_THEME_IMG_URL.'/next2.png" alt=""></a>'.PHP_EOL;



    if ($str)
        return "<div class=\"page_wrap\">{$str}</div>";
    else
        return "";
...
function get_list($write_row, $board, $skin_url, $subject_len=40)
{
    ...
    if (strstr($list['wr_option'], 'secret')){
        ...
        $list['icon_secret'] = '<img src="'.G5_THEME_IMG_URL.'/lock.png"> ';
	}
...
function member_delete($mb_id)
{
    ...
    $sql = " update {$g5['member_table']} set mb_password = '', mb_level = 1, mb_email = '', mb_homepage = '', mb_tel = '', mb_hp = '', mb_zip1 = '', mb_zip2 = '', mb_addr1 = '', mb_addr2 = '', mb_addr3 = '', mb_point = 0, mb_profile = '', mb_birth = '', mb_sex = '', mb_signature = '', mb_leave_date = '".date('Ymd', G5_SERVER_TIME)."', mb_memo = '".date('Ymd', G5_SERVER_TIME)." 삭제함\n".sql_real_escape_string($mb['mb_memo'])."', mb_certify = '', mb_adult = 0, mb_dupinfo = '' where mb_id = '{$mb_id}' ";
