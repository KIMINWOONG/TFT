...
function formatMembershipData($row) {
    ...
    // 회원명 조회
    $member_sql = "SELECT mb_name FROM `{$g5['member_table']}` WHERE mb_id = '{$row['mb_member_id']}'";
    $member_result = sql_fetch($member_sql);
    $row['mb_name'] = $member_result ? $member_result['mb_name'] : '';
...
function exportToExcel() {
    ...
    try {
        ...
        // 헤더 출력
        $headers = array('회원ID', '회원명', '회비종류', '내용', '금액', '유효기간', '납부예정일', '상태', '등록일', '결제방법', '관리자메모');
        fputcsv($output, $headers);
        
        // 데이터 출력
        while ($row = sql_fetch_array($result)) {
            // 회원명 조회
            $member_sql = "SELECT mb_name FROM `{$g5['member_table']}` WHERE mb_id = '{$row['회원ID']}'";
            $member_result = sql_fetch($member_sql);
            $member_name = $member_result ? $member_result['mb_name'] : '-';
