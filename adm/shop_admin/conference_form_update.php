...
$year_dir = $conference_dir."/".$sy_year;
 ...
 */
    ...
    try {
        ...
        if (!$result) {
            throw new Exception('집담회 삭제 중 오류가 발생했습니다: '.mysqli_error($g5['connect_db']));
        }
...
// 리다이렉트 처리
if ($w == "" || $w == "u") {
    $msg = ($w == "") ? "집담회가 생성되었습니다." : "집담회가 수정되었습니다.";
    alert($msg, "./conference_form.php?w=u&sy_id=$sy_id");
} else {
    alert("집담회가 삭제되었습니다.", "./conference_list.php");
}
?>